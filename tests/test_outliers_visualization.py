...
class TestVisualizeOutliersHist:
    ...
    def test_visualize_outliers_hist_basic(self, mock_show):
        ...
        # Should not raise
        visualize_outliers_hist(data_cleaned, data_original, contamination=0.03)
    ...
    def test_visualize_outliers_hist_with_columns(self, mock_show):
        ...
        visualize_outliers_hist(
            data_cleaned, data_original, columns=["A"], contamination=0.03
        )
    ...
    def test_visualize_outliers_hist_custom_figsize(self, mock_show):
        ...
        visualize_outliers_hist(
            data_cleaned, data_original, figsize=(15, 8), contamination=0.03
        )
    ...
    def test_visualize_outliers_hist_custom_bins(self, mock_show):
        ...
        visualize_outliers_hist(
            data_cleaned, data_original, bins=100, contamination=0.03
        )
    ...
    def test_visualize_outliers_hist_missing_column(self):
        ...
        np.random.seed(42)
        data_original = pd.DataFrame({"A": np.random.normal(0, 1, 100)})
        data_cleaned = data_original.copy()

        with pytest.raises(ValueError, match="Columns not found"):
            visualize_outliers_hist(
                data_cleaned, data_original, columns=["NonExistent"]
            )
...
class TestVisualizeOutliersPlotlyScatter:
    ...
    @patch("spotforecast2.plots.outlier_plots.go")
    def test_visualize_outliers_plotly_scatter_basic(self, mock_go):
        ...
        visualize_outliers_plotly_scatter(
            data_cleaned, data_original, contamination=0.03
        )
    ...
    def test_visualize_outliers_plotly_scatter_no_plotly(self):
        ...
        with patch("spotforecast2.plots.outlier_plots.go", None):
    ...
    def test_visualize_outliers_plotly_scatter_missing_column(self):
        ...
        data_original = pd.DataFrame({"A": np.random.normal(0, 1, 100)}, index=dates)
        data_cleaned = data_original.copy()
...
class TestIntegration:
    ...
    def test_outlier_workflow(self, mock_go, mock_show):
        ...
        outliers = get_outliers(data_original, contamination=0.015)
        ...
        visualize_outliers_hist(data_original, data_original, contamination=0.015)

        assert mock_show.called

        # Step 3: Visualize with Plotly
        visualize_outliers_plotly_scatter(
            data_original, data_original, contamination=0.015
        )
    ...
    def test_outlier_detection_reproducibility(self):
        ...
        pd.testing.assert_series_equal(
            outliers1["A"].sort_index(), outliers2["A"].sort_index()
        )
