...
class TestGetOutliersBasic:
    ...
    def test_get_outliers_with_different_contamination(self, sample_data_simple):
        ...
        assert len(outliers_liberal['temperature']) >= len(outliers_conservative['temperature'])
    ...
    def test_get_outliers_reproducibility(self, sample_data_simple):
        ...
        """
        outliers1 = get_outliers(sample_data_simple, random_state=42, contamination=0.03)
        outliers2 = get_outliers(sample_data_simple, random_state=42, contamination=0.03)
    ...
    def test_get_outliers_different_random_state(self, sample_data_simple):
        ...
        outliers1 = get_outliers(sample_data_simple, random_state=42, contamination=0.03)
        outliers2 = get_outliers(sample_data_simple, random_state=123, contamination=0.03)
...
class TestRandomStateParameter:
    ...
    def test_random_state_same_state_produces_same_results(self, sample_data_simple):
        ...
        results = []
        for _ in range(3):
            outliers = get_outliers(sample_data_simple, random_state=42, contamination=0.03)
    ...
    def test_random_state_edge_cases(self, sample_data_simple):
        ...
        for state in [0, 1, 42, 1234, 9999]:
            outliers = get_outliers(sample_data_simple, random_state=state, contamination=0.03)
...
class TestEdgeCases:
    ...
    def test_small_dataframe(self):
        ...
        outliers = get_outliers(data, contamination=0.25)  # High contamination for tiny dataset
    ...
    def test_large_dataframe(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame({
            'A': np.concatenate([np.random.normal(0, 1, 10000), np.random.normal(10, 1, 100)]),
            'B': np.concatenate([np.random.normal(5, 2, 10000), np.random.normal(20, 2, 100)])
        })
    ...
    def test_with_very_small_values(self):
        ...
        data = pd.DataFrame({
            'A': np.concatenate([
                np.random.normal(1e-10, 1e-11, 100),
                [1e-8, 1e-8, 1e-8]
            ])
        })
