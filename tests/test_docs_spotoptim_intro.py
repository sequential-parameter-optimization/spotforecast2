...
def test_spotoptim_intro_simple_tuning():
    """Validates the Simple Tuning Example from spotoptim_intro.qmd."""
...
def test_spotoptim_intro_advanced_tuning():
    """Validates the Advanced Tuning Example from spotoptim_intro.qmd."""
    # Data generation
    y_adv = pd.Series(
    ...
    forecaster_rf = ForecasterRecursive(
        estimator=RandomForestRegressor(random_state=123), lags=12
    )
    ...
    assert isinstance(results_adv, pd.DataFrame)
    assert len(results_adv) == 10
    assert "n_estimators" in results_adv.columns
    assert "max_depth" in results_adv.columns
    assert "lags" in results_adv.columns
    assert "mean_absolute_error" in results_adv.columns
    assert "mean_squared_error" in results_adv.columns
