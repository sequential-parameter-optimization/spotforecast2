...
class TestWeightFunctionPickling:
    ...
    def test_weight_function_no_closure_dependency(self, sample_weights_series):
        ...
        assert (
            not hasattr(weight_func, "__closure__") or weight_func.__closure__ is None
        )
...
class TestWeightFunctionWithForecaster:
    ...
    def test_weight_function_preserves_magnitude_after_pickle(
        self, sample_weights_series
    ):
        ...
        np.testing.assert_almost_equal(
            original_result.sum(), restored_result.sum(), decimal=10
        )
...
class TestDocumentationExamples:
    ...
    def test_ensemble_weights_example(self):
        ...
        performance_scores = pd.Series([0.95, 0.88, 0.92, 0.85], index=[0, 1, 2, 3])
    ...
    def test_missing_data_weights_example(self):
        ...
        weights = pd.Series(
            [1.0, 0.9, 0.8], index=[0, 1, 2]  # High certainty -> weight 1.0, low -> 0.8
        )
...
class TestEdgeCasesAndRobustness:
    ...
    def test_weight_function_with_large_series(self):
        ...
        large_weights = pd.Series(np.random.rand(10000), index=range(10000))
