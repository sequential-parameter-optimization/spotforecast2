...
# positive.  spotforecast2 inherits its pipeline steps cross-package from
# spotforecast2_safe, hence both prefixes.
ECOSYSTEM_PREFIXES = ("spotforecast2", "spotforecast2_safe")
...
# documented on their OWN canonical API page — here, the spotforecast2-safe
# site — and which quartodoc/griffe cannot render onto subclass pages because
# the base is reached through the ``spotforecast2_safe.forecaster.wrappers``
# re-export alias (griffe resolves neither statically nor dynamically).  A
# member inherited from one of these owners is therefore "documented" already.
# See ADR 2026-06-15-api-coverage-gate.md.
DOCUMENTED_ELSEWHERE: dict[str, str] = {
    "spotforecast2_safe.forecaster.wrappers.model.ForecasterRecursiveModel": (
        "documented on the spotforecast2-safe API site; unreachable via the "
        "forecaster.wrappers re-export alias"
    ),
    "spotforecast2_safe.forecaster.wrappers.lgbm.ForecasterRecursiveLGBM": (
        "documented on the spotforecast2-safe API site; unreachable via the "
        "forecaster.wrappers re-export alias"
    ),
    "spotforecast2_safe.forecaster.wrappers.xgb.ForecasterRecursiveXGB": (
        "documented on the spotforecast2-safe API site; unreachable via the "
        "forecaster.wrappers re-export alias"
    ),
}
